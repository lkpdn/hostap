...
static void
ieee802_1x_stale_participants_retire(
		struct ieee802_1x_mka_participant *participant)
{
	...
	/* This works around otherwise infinitely increasing number of
	 * duplicate MKA instances, which covers the same SCs in the case
	 * Reauth Timer is enabled. The specs mention its necessity when
	 * the PAE (or PACP in 802.1X-2010 state machine) moves from
	 * AUTHENTICATED to RESTART but nothing about when to cease the
	 * old one.
	 */
...
ieee802_1x_kay_elect_key_server(struct ieee802_1x_mka_participant *participant)
{
	...
	if (i_is_key_server) {
		...
		if (!sci_equal(&kay->key_server_sci, &key_server->sci)) {
			ieee802_1x_cp_signal_chgdserver(kay->cp);
			ieee802_1x_cp_sm_step(kay->cp);
		} else if (!participant->principal) {
			/* incite duplicate old participant retirement */
			ieee802_1x_stale_participants_retire(participant);
		}
